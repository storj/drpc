...
type drpc struct{ *protogen.GeneratedFile }
...
func (d *drpc) generateService(service *protogen.Service) {
	...
	d.P()
	d.P("func (", d.ServerDesc(service), ") Method(n int) (string, ", d.Ident("storj.io/drpc", "Receiver"), ", interface{}, bool) {")
	d.P("switch n {")
	for i, method := range service.Methods {
		d.P("case ", i, ":")
		d.P("return ", d.RPCGoString(method), ",")
...
func (d *drpc) generateClientMethod(method *protogen.Method) {
	...
	if !method.Desc.IsStreamingServer() && !method.Desc.IsStreamingClient() {
		d.P("out := new(", outType, ")")
		d.P("err := c.cc.Invoke(ctx, ", d.RPCGoString(method), ", in, out)")
	...
	d.P("stream, err := c.cc.NewStream(ctx, ", d.RPCGoString(method), ")")
	d.P("if err != nil { return nil, err }")
